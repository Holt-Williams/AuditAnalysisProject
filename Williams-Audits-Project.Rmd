...
This is the output of G. Holt Williams' Audits Data Analyst project. This was created using R Markdown, and can be navigated using the tabs below. Each dataset is within its own tab set, and the variables are split into further tabs under each dataset.
...
```{r Clean Audit Type, echo=FALSE}
label<-"Audit Type"
counts<- count(CSQA%>%filter(CSQA[[8]]!= "(Unknown)"),vars=CSQA[[8]])
percent<-prop.table(counts$n)
df<-as.data.frame(counts)
df$perc<-percent
df
...
```{r Audit Method Clean, echo=FALSE}
label<-"Audit Method"
counts<- count(CSQA%>%filter(CSQA[[9]] != "(Unknown)",CSQA[[9]] != "None"),vars=CSQA[[9]])
percent<-prop.table(counts$n)
df<-as.data.frame(counts)
df$perc<-percent
df
...
```{r Proposed Quarter:clean, echo=FALSE}
label<-"Proposed Quarter"
counts<- count(CSQA%>%filter(CSQA[[10]] != "(Unknown)"),vars=CSQA[[10]])
percent<-prop.table(counts$n)
df<-as.data.frame(counts)
df$perc<-percent
df
...
## WorkSheet Analysis: Part III
  ...
  Without further information on the origin of the two datasets, I would recommend to not merge the two datasets. One reason is size diffrence of the two files. The GWP dataset has 96 rows versus the CSQA has 12 rows, which would mean the merged dataset would be overly influenced by the GWP dataset. Other reasons is the diffrence in the means for the diffrent variables. Along the simialr idea, the means and medians of several of the variables are not close, and therefore the GWP dataset woudl also have a large influence on a merged dataset.

  While the above are reasons to not merge the datasets, it might be useful to merge them for the ease of data analysis. If the merged datasets contain an extra column with a variable indicating its origin, automated analysis would likely be easier as a simple filter can choose which data set to depict.
